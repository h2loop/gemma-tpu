...
\begin{abstract}
We present a comprehensive technical account of fine-tuning Gemma 4 31B using LoRA on Google Cloud TPU v5p-8, and serving the fine-tuned model for inference on TPU v6e-8 (Trillium). We document the specific code-level adaptations required to port the GPU training recipe (PyTorch + HuggingFace TRL + FSDP) to the JAX + Tunix/Qwix stack, including mesh configuration, LoRA module naming differences, sharding annotation fixes, gradient checkpointing, the data pipeline, and a custom Orbax-to-safetensors checkpoint merging procedure. For inference, we cover the vLLM-TPU Docker setup required to serve Gemma 4 on v6e-8 and explain the observed latency and throughput characteristics across a QPS sweep spanning 512 to 16k input tokens. Across both workloads we compare performance and cost against a 2$\times$H100 GPU baseline running identical hyperparameters. The TPU completes training 1.61$\times$ faster at 2.12$\times$ lower cost. For inference, TPU v6e-8 matches GPU at short context ($\leq$2048 tokens) and decisively outperforms at long context: 66\% higher throughput and 23.6$\times$ faster TTFT at 4096-token inputs (61\,ms vs 1,443\,ms at QPS=4), driven by higher aggregate compute and 2$\times$ more KV cache capacity (250 GB vs 160 GB total HBM). End-to-end, TPU is 1.82$\times$ cheaper for a representative train-plus-serve workload at short context, and substantially cheaper at long context where TPU's per-token cost advantage reaches 41\%.
\end{abstract}
...
Host RAM & \textasciitilde355 GB & \textasciitilde494 GB \\
...
\begin{itemize}[nosep]
  ...
  \item \textbf{No grad clipping by default}: PyTorch's HuggingFace Trainer applies \texttt{max\_grad\_norm=1.0} automatically (confirmed in GPU config). On TPU we omit this; \texttt{optax.clip\_by\_global\_norm(1.0)} can be prepended to the chain if needed.
...
The TPU recipe uses Orbax (\texttt{orbax-checkpoint}) with a GCS path for checkpoint storage. Checkpoints are saved every 100 optimizer steps, retaining the 3 most recent (matching the GPU recipe's \texttt{save\_steps=500} cadence (GPU has no \texttt{save\_total\_limit}) with a finer cadence for TPU's longer run). Checkpoints survive spot preemption because they are written directly to GCS rather than local disk.
...
Both runs trained for 1 epoch on 10K CodeV-R1 samples with identical LoRA hyperparameters: rank=64, $\alpha$=64, dropout=0.05, AdamW with cosine LR schedule (peak 1e-4, 100 warmup steps, decay to 0). The two runs differ in effective batch size and sequence length: TPU used effective batch=8 with max\_seq\_len=3,072 (1,244 optimizer steps); GPU used effective batch=16 with max\_seq\_len=8,192 (625 optimizer steps).
...
Final training loss & \textbf{0.125} & 2.258 & TPU \\
...
\begin{figure}[H]
  ...
  \caption{Training loss over the full epoch (x-axis normalized to training progress 0--1). Both runs use assistant-only loss masking. The absolute loss difference (0.125 vs 2.258 final) largely reflects different sequence lengths and tokenization dynamics: the TPU trained at max\_seq\_len=3,072 with smaller effective batch=8, while the GPU trained at max\_seq\_len=8,192 with effective batch=16. The y-axis of the GPU curve is linearly rescaled to match the TPU's start/end range for visual comparison.}
  \label{fig:loss}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Gradient norm over training (log scale). The TPU shows a large early spike (497 at step 66) during warmup, then stabilises. The GPU shows a spike of 106.7 at step 110, likely from a difficult batch at the learning rate peak. Both runs are stable for the bulk of training.}
  \label{fig:gradnorm}
\end{figure}
...
\begin{enumerate}[nosep]
  \item \textbf{More chips}: 4 TPU v5p chips run in tensor-parallel vs 2 H100 GPUs, providing more aggregate compute and memory bandwidth for the same model.
  \item \textbf{ICI interconnect}: TPU chips communicate via a dedicated ICI (inter-chip interconnect) fabric, which avoids PCIe bottlenecks present in GPU configurations without NVLink.
...
Checkpoint step & 1,244 (final) & 625 (final) \\
...
\begin{figure}[H]
  ...
  \caption{Distribution of pass@1 scores across 156 problems. GPU-trained model: 90 problems at 100\%, 29 at 0\%. TPU-trained model: 80 at 100\%, 32 at 0\%. The gap is primarily in the full-pass bucket (10 fewer problems solved consistently by TPU).}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{Sequence length and batch size}: The GPU trained at max\_seq\_len=8,192 with effective batch=16, vs TPU at max\_seq\_len=3,072 with effective batch=8. Longer sequences allow the model to see more complete reasoning traces, which may improve performance on complex problems.
\end{enumerate}
...
The extended benchmark uses \texttt{vllm bench serve} with a random dataset, temperature=0, sweeping QPS from 1 to 64 across multiple input length regimes (512--15360 tokens input, 256--512 tokens output). Both servers configured with \texttt{max\_model\_len=4096} for the baseline run; extended runs used higher context lengths. TPU uses TP=8; GPU uses TP=2. Both use default KV cache dtype (bfloat16).
...
KV cache dtype & bfloat16 (default) & bfloat16 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
At short input lengths ($\leq$2048 tokens), GPU edges ahead by $\sim$6\% on peak output throughput. This is the \emph{memory-bandwidth-bound} decode regime. With only 2 chips, H100 has high per-chip HBM bandwidth, and with short KV caches the bottleneck is weight loading from HBM rather than KV state---a regime where fewer, higher-bandwidth chips can outperform more chips with lower individual bandwidth.
...
\begin{enumerate}
  ...
  \item \textbf{Higher total HBM for KV cache}: The v6e-8 has 250 GB total HBM vs the GPU's 160 GB. After subtracting model weights (\textasciitilde62 GB), the TPU has \textasciitilde188 GB available for KV cache vs the GPU's \textasciitilde98 GB --- nearly 2$\times$ more. This allows the TPU to hold more concurrent long-context requests before eviction. The GPU hits KV cache pressure at $\sim$8 concurrent long requests (QPS saturation at 8); the TPU sustains up to $\sim$32.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Memory headroom}: 411 GB HBM (training), 250 GB HBM (inference) enables 2$\times$ more KV cache capacity than GPU's 160 GB
\end{enumerate}
...
\textbf{Inference}: TPU is the clear winner at long-context workloads. At 4096-token inputs, TPU delivers 66\% higher throughput and 23.6$\times$ faster TTFT. At short context ($\leq$2048 tokens), performance is comparable with a marginal GPU throughput edge (+6\%). The larger HBM (250 GB vs 160 GB) gives TPU nearly 2$\times$ more KV cache capacity, enabling 4$\times$ higher QPS saturation at medium context.
